...
func sortByLastChecked(left: Category?, right: Category?) -> Bool {
    ...
    let lastCheckedLeft = left!.items.compactMap { $0.lastCheckedAt }.max() ?? Date.distantPast
    let lastCheckedRight = right!.items.compactMap { $0.lastCheckedAt }.max() ?? Date.distantPast
